...
patched &&= patch_file(helpers_path) do |c|
  if c.include?("'15.0'")
    puts "ℹ️ helpers.rb already patched"
  elsif c.include?('def self.min_xcode_version_supported')
...
patched &&= patch_file(utils_path) do |c|
  if c.include?('Skipping XCode version check (patched)')
    puts "ℹ️ utils.rb already patched"
  elsif c =~ /def self\.check_minimum_required_xcode.*?end/m
...
RUBY
    ...
    puts "⚠️ utils.rb: check method pattern not found (continuing)"
  end
end
